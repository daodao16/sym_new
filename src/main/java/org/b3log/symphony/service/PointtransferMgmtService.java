...
public class PointtransferMgmtService {
    ...
    public synchronized String transfer(final String fromId, final String toId, final int type, final int sum,
    	...
    	return this.transfer(fromId, toId, type, sum, sum, dataId, time, memo);
    }
     ...
     * @param toId   	the specified to id, may be system "sys"
     * @param type   	the specified type
     * @param fromSum   the specified fromSum
     * @param toSum   	the specified toSum
     * @param dataId 	the specified data id
     * @param time   	the specified time
     * @param memo   	the specified memo
     * @return transfer record id, returns {@code null} if transfer failed
     */
    public synchronized String transfer(final String fromId, final String toId, final int type, final int fromSum, 
    		final int toSum, final String dataId, final long time, final String memo) {
        ...
        try {
            ...
            if (!Pointtransfer.ID_C_SYS.equals(fromId)) {
                ...
                fromBalance = fromUser.optInt(UserExt.USER_POINT) - fromSum;
                if (fromBalance < 0) {
                    throw new Exception("Insufficient balance");
                }

                fromUser.put(UserExt.USER_POINT, fromBalance);
                fromUser.put(UserExt.USER_USED_POINT, fromUser.optInt(UserExt.USER_USED_POINT) + fromSum);
            ...
            if (!Pointtransfer.ID_C_SYS.equals(toId)) {
                ...
                toBalance = toUser.optInt(UserExt.USER_POINT) + toSum;
            ...
            pointtransfer.put(Pointtransfer.FROM_SUM, fromSum);
            pointtransfer.put(Pointtransfer.TO_SUM, toSum);
