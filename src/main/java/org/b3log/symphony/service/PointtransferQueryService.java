            ...
            for (int i = 0; i < records.length(); i++) {
                ...
                switch (type) {
                    ...
                    case Pointtransfer.TRANSFER_TYPE_C_INIT:
                        ...
                        break;
                    case Pointtransfer.TRANSFER_TYPE_C_FIN_GUIDE:
                    	desTemplate = desTemplate.replace("{point}", record.optString(Pointtransfer.SUM));
                    ...
                    case Pointtransfer.TRANSFER_TYPE_C_ADD_ARTICLE:
                        ...
                        break;
                    case Pointtransfer.TRANSFER_TYPE_C_ADD_ARTICLE_QNA:
                    	final JSONObject addArticleQNA = articleRepository.get(dataId);
                        if (null == addArticleQNA) {
                            desTemplate = langPropsService.get("removedLabel");

                            break;
                        }

                        final String addArticleQNALink = "<a href=\""
                                + addArticleQNA.optString(Article.ARTICLE_PERMALINK) + "\">"
                                + Escapes.escapeHTML(addArticleQNA.optString(Article.ARTICLE_TITLE)) + "</a>";
                        desTemplate = desTemplate.replace("{article}", addArticleQNALink);

                        break;
                    case Pointtransfer.TRANSFER_TYPE_C_ADD_ARTICLE_DISCUSS:
                    	final JSONObject addArticleDiscuss = articleRepository.get(dataId);
                        if (null == addArticleDiscuss) {
                            desTemplate = langPropsService.get("removedLabel");

                            break;
                        }

                        final String addArticleDiscussLink = "<a href=\""
                                + addArticleDiscuss.optString(Article.ARTICLE_PERMALINK) + "\">"
                                + Escapes.escapeHTML(addArticleDiscuss.optString(Article.ARTICLE_TITLE)) + "</a>";
                        desTemplate = desTemplate.replace("{article}", addArticleDiscussLink);

                        break;
                    case Pointtransfer.TRANSFER_TYPE_C_ADD_ARTICLE_TAG:
                    	final JSONObject addArticleUseTag = articleRepository.get(dataId);
                        if (null == addArticleUseTag) {
                            desTemplate = langPropsService.get("removedLabel");

                            break;
                        }

                        final String addArticleUseTagLink = "<a href=\""
                                + addArticleUseTag.optString(Article.ARTICLE_PERMALINK) + "\">"
                                + Escapes.escapeHTML(addArticleUseTag.optString(Article.ARTICLE_TITLE)) + "</a>";
                        desTemplate = desTemplate.replace("{article}", addArticleUseTagLink);
                        desTemplate = desTemplate.replace("{point}", record.optString(Pointtransfer.SUM));
