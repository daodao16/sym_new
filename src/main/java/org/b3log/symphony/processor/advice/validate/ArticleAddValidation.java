...
public class ArticleAddValidation extends ProcessAdvice {
    ...
    public static void validateArticleFields(final RequestContext context, final JSONObject requestJSONObject) throws RequestProcessAdviceException {
        ...
        String articleTagsTemp = requestJSONObject.optString(Article.ARTICLE_TAGS);
        if (StringUtils.isBlank(articleTagsTemp)) {
            // 发帖时标签改为非必填 https://github.com/b3log/symphony/issues/811
            articleTags = "待分类";
        }
        //articleTags = Tag.formatTags(articleTags);
        articleTags = articleTagsTemp.replaceAll("\\s+", "").replaceAll("，", ",").replaceAll("、", ",").
...
//        }
        ...
        if (StringUtils.isNotBlank(articleTagsTemp)) {
            ...
            requestJSONObject.put(Article.ARTICLE_TAGS, Tag.formatTags(tagBuilder.toString()));
        }
