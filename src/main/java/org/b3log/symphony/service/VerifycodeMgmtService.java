...
public class VerifycodeMgmtService {
     ...
     * regist sms template
     */
    private static final String TEMLATE_REGISTE = "SMS_159783784";
    
    /**
     * reset password sms template
     */
    private static final String TEMLATE_RESET_PASSWORD = "SMS_159783787";
    ...
    /**
     * send mobile vertify code
     * 
     * @param verifycode
     */
    @Transactional
	public void sendMobileVerifycode(JSONObject verifycode) throws ServiceException {
		// 检查校验码是否已经存在
		final String mobile = verifycode.optString(Verifycode.RECEIVER);
        final int bizType = verifycode.optInt(Verifycode.BIZ_TYPE);
         
        final List<Filter> filters = new ArrayList<>();
        filters.add(new PropertyFilter(Verifycode.RECEIVER, FilterOperator.EQUAL, mobile));
        filters.add(new PropertyFilter(Verifycode.BIZ_TYPE, FilterOperator.EQUAL, bizType));
        filters.add(new PropertyFilter(Verifycode.TYPE, FilterOperator.EQUAL, Verifycode.TYPE_C_MOBILE));
        final Query query = new Query().setFilter(new CompositeFilter(CompositeFilterOperator.AND, filters));
        
		try {
			JSONObject result = verifycodeRepository.get(query);
			final JSONArray verifycodes = result.optJSONArray(Keys.RESULTS);
			
			//该手机该类型尚无验证码记录
			if (0 == verifycodes.length()) {
				String oId =  verifycodeRepository.add(verifycode);
				verifycode.put(Keys.OBJECT_ID, oId);
	        } else {
	        	//有多条的话，就有问题了，这个可能性还是有的，数据库上要想办法
	        	if(1 < verifycodes.length()) {
	        		final String msg = "send verifycode failed , duplicated record";
	                LOGGER.log(Level.ERROR, msg);

	                throw new ServiceException(msg);
	        	}
	        	//只有一条记录的话，修改验证码数据，发送
	        	JSONObject verifycodeOld = verifycodes.optJSONObject(0);
	        	verifycode.put(Keys.OBJECT_ID, verifycodeOld.optString(Keys.OBJECT_ID));
	        }
			
			//以下是发送验证码部分，调用阿里云，必须是异步的!!!
     ...
     * @param verifycode
     */
	private void sendSms(JSONObject verifycode) {
		int bizType = verifycode.optInt(Verifycode.BIZ_TYPE);
		String mobile = verifycode.optString(Verifycode.RECEIVER);
		JSONObject paramObjec = new JSONObject();
		//业务比较简单，就这样处理一下算了。
		switch (bizType) {
	        case Verifycode.BIZ_TYPE_C_REGISTER:
	        	paramObjec.put(Verifycode.CODE, verifycode.optString(Verifycode.CODE));
	        	aliSmsSendService.sendAsyn(mobile, TEMLATE_REGISTE, paramObjec);
	            break;
	        case Verifycode.BIZ_TYPE_C_RESET_PWD:
	        	paramObjec.put(Verifycode.CODE, verifycode.optString(Verifycode.CODE));
	        	aliSmsSendService.sendAsyn(mobile, TEMLATE_RESET_PASSWORD, paramObjec);
	            break;
	        default:
	            LOGGER.warn("Send sms verify code failed with wrong biz type [" + bizType + "]");
		}
	}
}
