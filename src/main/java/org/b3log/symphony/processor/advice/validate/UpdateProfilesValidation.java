...
public class UpdateProfilesValidation extends ProcessAdvice {
     ...
     */
    public static final int MAX_USER_INTRO_LENGTH = 80;
    ...
    public void doAdvice(final RequestContext context) throws RequestProcessAdviceException {
        ...
        String userTagsTemp = requestJSONObject.optString(UserExt.USER_TAGS);
        if (StringUtils.isNotBlank(userTagsTemp)) {
            //userTags = Tag.formatTags(userTags);
        	String userTags = userTagsTemp.replaceAll("\\s+", "").replaceAll("，", ",").replaceAll("、", ",").
        			replaceAll("；", ",").replaceAll(";", ",");
            String[] tagTitles = userTags.split(",");
            if (null == tagTitles || 0 == tagTitles.length || tagTitles.length > MAX_TAG_COUNT) {
            ...
            requestJSONObject.put(UserExt.USER_TAGS, Tag.formatTags(tagBuilder.toString()));
        }
    }
