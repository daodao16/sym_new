...
public class VerifycodeQueryService {
    ...
    /**
     * get a vertifycode with specified type,bizType and reciver
     * @param type the send type
     * @param bizType biztype
     * @param reciver
     * @return verifycode, returns {@code null} if not found
     */
	public JSONObject getVerifycodeByReciver(int type,int bizType, String reciver) {
		final Query query = new Query().setFilter(CompositeFilterOperator.and(
                new PropertyFilter(Verifycode.TYPE, FilterOperator.EQUAL, type),
                new PropertyFilter(Verifycode.BIZ_TYPE, FilterOperator.EQUAL, bizType),
                new PropertyFilter(Verifycode.RECEIVER, FilterOperator.EQUAL, reciver)));
