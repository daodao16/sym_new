...
public class UserQueryService {
    ...
    /**
     * Gets a user by the specified user mobile.
     *
     * @param mobile the specified user mobile
     * @return for example,      <pre>
     * {
     *     "oId": "",
     *     "userName": "",
     ...
     * </pre>, returns {@code null} if not found
     */
	public JSONObject getUserByMobile(String mobile) throws ServiceException {
		try {
            return userRepository.getByMobile(mobile);
        } catch (final RepositoryException e) {
            LOGGER.log(Level.ERROR, "Gets user by mobile[" + mobile + "] failed", e);
            throw new ServiceException(e);
        }
	}
}
