...
public class ArticleMgmtService {
    ...
    public synchronized String addArticle(final JSONObject requestJSONObject) throws ServiceException {
        ...
        //add by daodao at 20190308 opt tags
        ...
        //add by daodao at 20190308 tag point;
        int tagPoint = 0;
...
//                final int addition = (int) Math.round(Math.sqrt(followerCnt));
            	//modify by daodao at 20190304为了兼容一下，以后会改过来的逻辑。
            	int addition = 0;
                ...
                //modify by daodao at 20190304 增加提问机要需要积分的验证逻辑
                final int qna = Article.ARTICLE_TYPE_C_DISCUSSION == articleType ?
                        Pointtransfer.TRANSFER_SUM_C_ADD_ARTICLE_DISCUSS : 0;
                
                final int discuss = Article.ARTICLE_TYPE_C_QNA == articleType ?
                        Pointtransfer.TRANSFER_SUM_C_ADD_ARTICLE_QNA : 0;
                //modify over

                final int sum = Pointtransfer.TRANSFER_SUM_C_ADD_ARTICLE + addition + rewardPoint + qnaOfferPoint 
                		+ broadcast + qna + discuss + tagPoint;
...
//                        Pointtransfer.TRANSFER_SUM_C_ADD_ARTICLE + addition, articleId, System.currentTimeMillis(), "");
                ...
                if (Article.ARTICLE_TYPE_C_DISCUSSION == articleType) {
                	pointtransferMgmtService.transfer(authorId, Pointtransfer.ID_C_SYS,
                            Pointtransfer.TRANSFER_TYPE_C_ADD_ARTICLE_DISCUSS,
                            Pointtransfer.TRANSFER_SUM_C_ADD_ARTICLE_DISCUSS, articleId, System.currentTimeMillis(), "");
                }
                
                if (Article.ARTICLE_TYPE_C_QNA == articleType) {
                	pointtransferMgmtService.transfer(authorId, Pointtransfer.ID_C_SYS,
                            Pointtransfer.TRANSFER_TYPE_C_ADD_ARTICLE_QNA,
                            Pointtransfer.TRANSFER_SUM_C_ADD_ARTICLE_QNA, articleId, System.currentTimeMillis(), "");
                }
            ...
            //Anonymous add by daodao at 20190307
            if (Article.ARTICLE_ANONYMOUS_C_ANONYMOUS == articleAnonymous) {
                pointtransferMgmtService.transfer(authorId, Pointtransfer.ID_C_SYS,
                      Pointtransfer.TRANSFER_TYPE_C_ADD_ARTICLE,
                      Pointtransfer.TRANSFER_SUM_C_ADD_ARTICLE_ANONYMOUS, articleId, System.currentTimeMillis(), "");
            }
            
            //Include special tags add by daodao at 20190308
            if(tagPoint > 0) {
    			pointtransferMgmtService.transfer(authorId, Pointtransfer.ID_C_SYS,
                        Pointtransfer.TRANSFER_TYPE_C_ADD_ARTICLE_TAG, tagPoint, 
    ...
    /**
     * Reserved tag point outcome
     * @param tagTitles article tags
     */
	private int tagReservedPoint(String[] tagTitles) {
		if(tagTitles.length == 0) {
			return 0;
		}
		String reservedTags = langPropsService.get("reservedTags");
		String reservedTagPoints = Symphonys.get("pointReservedTagPoints");
		
		if(reservedTags.length() == 0) {
			return 0;
		}
		String[] reservedTagsArray = reservedTags.split(",");
		String[] reservedTagPointsArray = reservedTagPoints.split(",");
		if(reservedTagsArray.length != reservedTagPointsArray.length) {
			return 0;
		}
		...
		for(String tagTitle : tagTitles) {
			for(String reservedTag : reservedTagsArray) {
				if(StringUtils.equals(tagTitle, reservedTag)) {
					tagPoint += Integer.parseInt(reservedTagPointsArray[i]);
					break;
				}
			}
			i++;
...
//            addition += (collectCnt + watchCnt) * 2;
            int thanksCnt = oldArticle.optInt(Article.ARTICLE_THANK_CNT, 0);
            addition += thanksCnt * 2;
            //mofify over
        ...
        try {
            ...
            if (Article.ARTICLE_ANONYMOUS_C_PUBLIC == article.optInt(Article.ARTICLE_ANONYMOUS)) {
                final boolean succ = null != pointtransferMgmtService.transfer(senderId, receiverId,
                        ...
                        Pointtransfer.TRANSFER_SUM_C_ARTICLE_THANK, Pointtransfer.TRANSFER_SUM_C_ARTICLE_BE_THANK,
